    ...
    private fun handleReceivedData(data: ByteArray) {
        ...
        try {
            if (data.size >= 12) {
                ...
                Log.d(TAG, "Parsed - Skin: $temperatureSkin, Outside: $temperatureOutside, Battery: $batteryLevel")

                // ALWAYS call the callback for debugging - ignore monitoring state temporarily
                mainHandler.post {
                    onDataReceived?.invoke(temperatureSkin, temperatureOutside, temperatureSkin, batteryLevel)
                }
            } else {
