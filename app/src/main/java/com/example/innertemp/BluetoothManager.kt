    ...
    private fun enableNotifications(gatt: BluetoothGatt, characteristic: BluetoothGattCharacteristic) {
        ...
        if (descriptor != null) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
                ...
                Log.d(TAG, "writeDescriptor (new API) result: $result for ${characteristic.uuid}")
            } else {
                descriptor.value = BluetoothGattDescriptor.ENABLE_NOTIFICATION_VALUE
                val result = gatt.writeDescriptor(descriptor)
                Log.d(TAG, "writeDescriptor (old API) result: $result for ${characteristic.uuid}")
            }
        } else {
    ...
    private fun handleReceivedData(data: ByteArray) {
        ...
        try {
            if (data.size >= 12) {
                ...
                Log.d(TAG, "Parsed data - Skin: $roundedTemperatureSkin, Outside: $roundedTemperatureOutside, Core: $roundedTemperatureCore, Battery: $roundedBatteryLevel")
                ...
                Log.e(TAG, "Data size is insufficient. Expected at least 12 bytes, got ${data.size}")
            }
        } catch (e: Exception) {
